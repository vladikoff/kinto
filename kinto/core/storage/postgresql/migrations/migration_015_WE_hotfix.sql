...
BEGIN
    -- Kinto WE hotfix.
    -- Since we don't need to synchronize the list of existing buckets, we
    -- do not care about buckets timestamps.
    IF uid = '' AND resource = 'bucket' THEN
...
DECLARE
    previous TIMESTAMP;
    current TIMESTAMP;
    is_not_bucket BOOLEAN;

BEGIN
    previous := NULL;

    -- Reading and updating the timestamp for (parent, collection_id) can
    -- be a serious bottleneck for buckets since they all have parent_id = ''.
    -- Each bucket creation will update the same row in the timestamps table.
    is_not_bucket := (NEW.parent_id <> '' AND NEW.collection_id <> 'bucket');
    IF is_not_bucket THEN
